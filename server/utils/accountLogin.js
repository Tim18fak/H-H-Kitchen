...
const validUserEmail =  async function(Db,email,username){
 ...
 if(!existAdmin || existAdmin.activationStatus === process.env.INVALIDSTATUS || existAdmin.username !== username) {
  return {value:false}
 }
...
const validateUserPass = async function(password,data) {
 ...
 if(!validatedEmail.value) return res.status(403).json({err: 'Invalid',errResponse: 97})

 const validatePass = await validateUserPass(password,validatedEmail.existAdmin)

 if(!validatePass) res.status(403).json({err: 'invalid',
errResponse: '97'})
